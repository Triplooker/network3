...
check_private_key() {
  ...
  for location in "${key_locations[@]}"; do
    if [ -f "$location" ]; then
      echo -e "\nНайден ключ в $location:"
      if [[ "$location" == *"wg0.conf" ]]; then
        grep "PrivateKey" "$location" | cut -d'=' -f2 | tr -d ' '
      else
        cat "$location"
      fi
      key_found=true
  ...
  if [ "$key_found" = false ]; then
    ...
    echo "Попробуйте переустановить ноду"
  fi
}
  ...
  if [ -d "$HOME/ubuntu-node" ]; then
    ...
    for log_file in "${log_files[@]}"; do
      if [ -f "$log_file" ]; then
        ...
        echo "2. С��едить за логами в реальном времени (Ctrl+C для выхода)"
    ...
    echo "❌ Директория ноды не найдена"
  fi
}
...
while true; do
    ...
    case $choice in
      ...
      9)
        ...
        ;;
      *)
        echo "Неверный пункт. Пожалуйста, выберите прав��льную цифру в меню."
        ;;
    esac
  done
